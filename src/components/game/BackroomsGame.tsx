...
export default function BackroomsGame({ initialSeed }: BackroomsGameProps) {
	...
	const showDesktopOverlay =
		!mobileControlsEnabled && !pointerLocked && !isDead;
	const effectiveMobileForward =
		mobileControlsEnabled && !isDead && mobileForward;
	const effectiveMobileBackward =
		mobileControlsEnabled && !isDead && mobileBackward;
	...
	useEffect(() => {
		...
		const flushFinish = () => {
			...
			const durationSeconds =
				(performance.now() - runRef.current.startMs) / 1000;
		...
		<main className="play-root" ref={rootRef}>
			<Canvas
				camera={{
					...
					fov: 74,
					near: 0.1,
					position: [0, GAME_CONFIG.eyeHeight, 0]
				}}
				dpr={graphics.dpr}
			...
			{showDesktopOverlay && (
				<div className="overlay-center">
					<div className="overlay-card">
						...
						<p>
							Move with WASD and mouse, left-click to throw keyboards, ESC to
							unlock.
						</p>
